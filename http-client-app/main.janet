...
# binary — the Jolt context built below, with app.core and its deps
# (jolt-lang/http-client + clj-http-lite) already loaded and compiled, is baked
# in at build time.
...
(defn- run-nrepl [addr]
  ...
  (print "http-client-app nREPL server started on " host ":" port)
  (flush)
  (defer (cleanup)
    (forever (ev/sleep 60))))
...
(defn main [& raw]
  ...
  (if (= "nrepl" (get argv 0))
    ...
    # Run -main on the event loop: this is a client app whose HTTP requests are
    # async (net/TLS yield to the loop), so unlike a blocking server it must be
    # driven by ev. Spawn it, then keep the loop alive until it exits.
    (do
      (ev/spawn
        (try
          (api/eval-string ctx (string "(apply app.core/-main " (clj-args argv) ")"))
          ([err] (eprint err)))
        (os/exit 0))
