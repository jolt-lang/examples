(ns gl-demo.core
  "App host: control panel + GLArea render loop. The shader lives in
   gl-demo.scene (data-notation modules composed with merge-specs)."
  ...
  [:vbox {:spacing 6 :margin 8}
   ...
   [:hbox {:spacing 12}
     [:checkbutton {:label "Smooth shading" :active @smooth
                    :on-toggled #(swap! smooth (fn [_] (pos? %)))}]
