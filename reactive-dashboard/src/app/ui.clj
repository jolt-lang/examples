...
(defn- diagram-panel
  "The event graph, fitted to the panel and movable.

  The whole block carries `data-ignore-morph`, which is what makes the zoom and
  the pan stick: the schema never changes at runtime, so there is nothing here
  for the SSE stream to update, and leaving it alone keeps the viewer's
  position from snapping back on the next patch. Zoom and offset live in
  datastar signals, and the transform on the inner group reads them, so the
  controls need no javascript of our own. The same flag lets a live re-render
  skip the drawing altogether, which keeps a few kilobytes of static SVG out of
  every patch the stream sends."
  [live?]
  (if live?
...
(defn- slider
  "A slider row owned entirely by the client.

  `data-ignore-morph` keeps the SSE stream out of it: the row's value lives in
  a datastar signal, so a re-render arriving mid-drag would otherwise morph the
  server's idea of the number over the one under the thumb. The readout is left
  empty for the same reason, since `data-text` fills it from the signal. The
  action is debounced, so dragging sends one request when the thumb settles
